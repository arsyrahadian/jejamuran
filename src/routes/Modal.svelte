<script>
  ...
  const expand = () => {
    ...
    shroomDesc.style.display = display
...
<div
  class="backdrop opening"
  bind:this={backdrop}
  on:click|self={closeModal}
  on:animationend={handleBackdrop}
...
>
  <div class="modal" bind:this={modal} on:animationend={handleModal}>
    ...
    <section class="inner-modal">
      <h4>{title}</h4>
      <div class="desc" bind:this={shroomDesc}>
        {@html desc}
      </div>
...
<style>
  ...
  :global(.modal .inner-modal .desc p) {
    font-weight: 400;
    font-size: 14px;
    line-height: 25.2px;
    color: #3d3d3d;
  ...
  :global(.modal .inner-modal .desc p a) {
    color: #3d3d3d;
  }
